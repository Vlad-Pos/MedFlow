...
const ModernCalendar = memo(({ onAppointmentClick, onTimeSlotClick }: ModernCalendarProps) => {
  ...
  // Load appointments with optimized query
  useEffect(() => {
    if (!user) return
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-800 dark:border-gray-700"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Simptome (opțional)</label>
                  <textarea
                    value={quickAddData.symptoms}
                    onChange={(e) => setQuickAddData({ ...quickAddData, symptoms: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-800 dark:border-gray-700"
                    rows={3}
                    placeholder="Descrierea simptomelor..."
                  />
                </div>
                ...
                <div className="flex space-x-3 pt-4">
                  <button
                    onClick={() => setShowQuickAdd(false)}
                    className="flex-1 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-800 transition-colors"
                  >
                    Anulează
                  </button>
                  <button
                  ...
                  >
                    ...
                    {dayAppointments.map((appointment) => (
                      <motion.div
                        ...
                        className={`absolute left-1 right-1 top-1 bottom-1 rounded-md p-2 text-xs text-white cursor-pointer group ${
              ...
              <div className="space-y-1">
                {dayAppointments.slice(0, 3).map((appointment) => (
                  <motion.div
                    ...
                    className={`p-1 rounded text-xs text-white cursor-pointer group ${getStatusColor(appointment.status)}`}
