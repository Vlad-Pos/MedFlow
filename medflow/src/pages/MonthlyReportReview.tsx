...
export default function MonthlyReportReview() {
  ...
  useEffect(() => {
    const checkPeriod = () => {
      setIsWithinSubmissionPeriod(isWithinSubmissionPeriod())
    }
  ...
  useEffect(() => {
    ...
    const loadAvailableMonths = async () => {
      try {
        ...
        showNotification('Eroare la încărcarea lunilor disponibile', 'error')
      }
    }
  ...
  useEffect(() => {
    ...
    const loadMonthlyData = async () => {
      ...
      try {
        ...
        showNotification('Eroare la încărcarea datelor lunare', 'error')
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleBulkApproval = () => {
    const eligibleReports = selectedReportIds.filter(id => {
      const report = monthlyReports.find(r => r.id === id)
      return report && report.status === 'final' && !report.isReadyForSubmission
    })

    if (eligibleReports.length === 0) {
      showNotification('Nu există rapoarte eligibile pentru aprobare', 'warning')
      return
    }

    setBulkApprovalDialog({
      isOpen: true,
      reportIds: eligibleReports,
      loading: false
    })
  }
  ...
  const handleCreateSubmissionBatch = () => {
