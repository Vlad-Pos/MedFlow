...
const goodSecurityPractices = [
  { pattern: 'allow read, write: if false', description: 'Default deny rule (good practice)' }
];
...
goodSecurityPractices.forEach(({ pattern, description }) => {
  if (rulesContent.includes(pattern)) {
    console.log(`✅ ${description}: Found '${pattern}'`);
  }
});
